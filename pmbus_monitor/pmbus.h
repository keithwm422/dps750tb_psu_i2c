...
class PMBus {
 ...
 public:
  ...
  float         V_in, I_in, V_out, I_out, T[3], fan[2], W_in, W_out;
  uint8_t       pmbus_revision, vout_mode;
  uint8_t       status_u8, status_vout, status_iout, status_input, status_temperature, 
                status_cml, status_other, status_mfr_specific, status_fans, *status_byte[STATUS_REGISTERS];
  uint16_t      status_word, vout_command;
  uint32_t      total_power_on;
...
*/
 ...
 private:
  ...
  int           fans, temperatures;
  short int     PSON_do, I2C_enable_do, output_direction, status_reg[STATUS_REGISTERS];
  float         vout_scale;
